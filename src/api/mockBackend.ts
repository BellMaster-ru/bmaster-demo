...
const computeAudioDuration = async (
  blob: Blob,
...
class MockBackend {
  ...
  private getScheduleForDate(date: Date): ScheduleRecord | null {
    ...
    return (
      this.state.school.schedules.find(
        (schedule) => schedule.id === scheduleId,
      ) || null
    );
  }

  private getDateMuteState(day: string): {
    mute_all: boolean;
    muted_lessons: Set<number>;
  } {
    const overrides = this.state.school.overrides.filter(
      (item) => item.at === day,
    );
  ...
  private enqueueSchedulerSound(soundName: string, eventKey: string): void {
    ...
    const sound = this.state.sounds.meta.find(
  ...
  private runSchedulerTick(): void {
    ...
    const fromMs =
      this.schedulerLastTickMs <= 0
        ? nowMs
        : Math.max(
            this.schedulerLastTickMs,
            nowMs - SCHEDULER_CATCH_UP_WINDOW_MS,
          );
    this.schedulerLastTickMs = nowMs;
    ...
    for (
      let lessonIndex = 0;
      lessonIndex < schedule.lessons.length;
      lessonIndex += 1
    ) {
      ...
      for (const eventType of ["start", "end"] as const) {
        const eventTime =
          eventType === "start" ? lesson.start_at : lesson.end_at;
  ...
  private resolveIdentityByToken(
    token: string | undefined | null,
  ): AuthIdentity | null {
    if (!token) {
      return null;
    }
    const session = this.state.auth.sessions.find(
  ...
  private toQueryAuthor(
    identity: AuthIdentity | null,
  ): QueryAuthor | undefined {
    if (!identity) {
      return undefined;
    }
  ...
  private trimQueryHistory(): void {
    ...
    const removable = Array.from(this.runtime.queries.values())
      .filter(
        (query) => query.status !== "playing" && query.status !== "waiting",
      )
    ...
    if (root === "school") {
      ...
      if (second === "assignments") {
        ...
        if ((ctx.method === "PATCH" || ctx.method === "DELETE") && third) {
          ...
          if (ctx.method === "DELETE") {
            this.state.school.assignments =
              this.state.school.assignments.filter(
      ...
      if (second === "overrides") {
        ...
        if (ctx.method === "POST" && !third) {
          ...
          for (const date of range) {
            const existing = this.state.school.overrides.find(
              (item) => item.at === date,
            );
            if (existing) {
    ...
    if (root === "lite") {
      if (second === "bells") {
        ...
        if (ctx.method === "PATCH" && third === "lessons" && ctx.segments[3]) {
          ...
          if (
            lessonId < 0 ||
            lessonId >= this.state.lite.bells.lessons.length
          ) {
          ...
          return this.response(
            200,
            jsonClone(this.state.lite.bells.lessons[lessonId]),
          );
        }
      }
